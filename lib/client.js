...
`
		...
		const INPUT_WS_ROUTE = '/api/ego/input/ws'
		const FRAMES_WS_ROUTE = '/api/ego/frames/ws'
		...
		// Long-lived WebSocket input channel — one connection per panel/tab
		// lifetime. Replaces the legacy per-event fetch('/api/ego/input') path
		// so pointermove (~60Hz) no longer opens a fresh HTTP request per frame.
		// The host (cast-server.js) proxies each inbound JSON frame to the
		// worker's /api/input and pushes the worker's response back as a JSON
		// message; the client only reacts to `capture-target-stale` (409) by
		// dropping the live target and refreshing, matching the old fetch
		// .then(res => res.status === 409) arm.
		//
		// Lazily connects on the first send(); reconnects 1.5s after a close;
		// drops messages while not yet OPEN (same fire-and-forget posture as
		// the previous fetch().catch(() => {}) — input is best-effort).
		function createInputSocket(opts) {
			var onStaleTarget = (opts && typeof opts.onStaleTarget === 'function') ? opts.onStaleTarget : function () {}
			var ws = null
			...
			function ensureSocket() {
				...
				var url = (location.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + location.host + INPUT_WS_ROUTE
				try { ws = new WebSocket(url) } catch (e) { ws = null; return }
				ws.onmessage = function (e) {
					try {
						var msg = JSON.parse(typeof e.data === 'string' ? e.data : '')
						if (!msg) return
						if (msg.status === 409 || (msg.body && msg.body.code === 'capture-target-stale')) onStaleTarget()
					} catch (err) {}
				}
				ws.onclose = function () {
					ws = null
					...
					reconnectTimer = window.setTimeout(function () { reconnectTimer = null; ensureSocket() }, 1500)
				}
				ws.onerror = function () { /* onclose handles reconnect */ }
			}

			function send(targetId, type, params) {
				if (disposed) return
				ensureSocket()
				if (!ws || ws.readyState !== WebSocket.OPEN) return
				try {
					ws.send(JSON.stringify(Object.assign({ targetId: targetId, type: type }, params || {})))
				} catch (e) {
		...
		return { send: send, dispose: dispose }
	}

	// Rolling-window end-to-end latency meter. Each inbound frame carries a
	// worker-side `ts` (Date.now() at capture); the panel pushes `Date.now() -
	// ts` per frame and reads p50/p95 over the last N samples. Same-machine
	// loopback means no clock skew to correct for. Used by the debug overlay
	// (Phase 0 baseline measurement) and to compare Phase 1/2 optimizations.
	function createLagMeter(maxSamples) {
		var samples = []
		var cap = Math.max(8, maxSamples | 0) || 64
		var frameCount = 0
		var windowStart = 0
		function push(ms) {
		...
		function percentile(p) {
			...
			var idx = Math.min(sorted.length - 1, Math.max(0, Math.floor((p / 100) * (sorted.length - 1))))
			return Math.round(sorted[idx])
		}
		function stats() {
			var now = Date.now()
			var elapsed = windowStart ? now - windowStart : 0
			return {
				p50: percentile(50),
				p95: percentile(95),
				last: samples.length ? Math.round(samples[samples.length - 1]) : null,
				count: samples.length,
				fps: elapsed > 0 ? Math.round((frameCount * 1000) / elapsed) : 0,
			}
		}
		function reset() { samples = []; frameCount = 0; windowStart = 0 }
		return { push: push, stats: stats, reset: reset }
	}

	// Binary screencast fast path: WebSocket replaces the SSE `frame` event.
	// The host (cast-server.js framesWss) reassembles length-prefixed packets
	// from the worker and emits each frame as a paired (text header, binary
	// JPEG) sequence; we attach the JPEG to an <img> via objectURL, skipping
	// the base64 encode/decode the SSE path pays (+33% size + per-frame
	// atob on the main thread).
	//
	// The SSE stream stays open for `spaces` / `capture-status` / `tool-call`
	// events; only `frame` events are dropped (via wsFrameActive) once the WS
	// path delivers its first frame. If the WS never opens or drops, the SSE
	// path resumes automatically — no client-side toggle.
	function createFrameChannel(opts) {
		var onFrame = (opts && typeof opts.onFrame === 'function') ? opts.onFrame : function () {}
		var onStatus = (opts && typeof opts.onStatus === 'function') ? opts.onStatus : function () {}
		var ws = null
		var reconnectTimer = null
		var disposed = false
		...
		function setActive(v) {
			if (active === v) return
			active = v
			try { onStatus(v) } catch (e) {}
		}

		function ensureSocket() {
			if (disposed) return
			if (ws && (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING)) return
			var url = (location.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + location.host + FRAMES_WS_ROUTE
			try { ws = new WebSocket(url) } catch (e) { ws = null; return }
			ws.binaryType = 'arraybuffer'
			ws.onmessage = function (e) {
				if (disposed) return
				if (typeof e.data === 'string') {
					try { pendingHeader = JSON.parse(e.data) } catch (err) { pendingHeader = null }
					return
				}
				...
				pendingHeader = null
				var jpegBuf = null
				if (e.data instanceof ArrayBuffer) jpegBuf = new Uint8Array(e.data)
				else if (e.data && e.data.buffer) jpegBuf = new Uint8Array(e.data.buffer)
				if (!jpegBuf) return
				setActive(true)
				try { onFrame(header, jpegBuf) } catch (err) {}
			}
			ws.onclose = function () {
				ws = null
				pendingHeader = null
	...
	function createMsePlayer(video, generation, mime, onFailure, onMetrics) {
		...
		mediaSource.addEventListener('sourceopen', function () {
			...
			sourceBuffer.addEventListener('updateend', function () {
				if (video.buffered.length) {
					...
					// Live-offset = how far the play head lags behind the newest
					// buffered byte. This is the dominant FFmpeg-path latency
					// component (Phase 0 baseline) and the main thing Phase 2's
					// WebCodecs direct-decode path will eliminate.
					if (typeof onMetrics === 'function') {
						try { onMetrics(Math.max(0, Math.round((end - video.currentTime) * 1000))) } catch (e) {}
					}
				}
				appendNext(); pump()
			})
	...
	// WebCodecs direct-decode player (Phase 2): bypass MSE's SourceBuffer
	// and <video> buffering entirely. The fMP4 stream from /api/ego/video
	// is demuxed in-JS (length-prefixed NALU samples extracted from
	// moof/mdat), fed to a VideoDecoder, and each output VideoFrame is
	// drawn to a <canvas> at rAF cadence. This removes the +100-300ms MSE
	// live-offset the legacy player accumulates (SourceBuffer append +
	// live-seek追尾). Falls back to createMsePlayer when VideoDecoder is
	// unavailable (Safari < 16.4, Firefox without flag, etc.).
	//
	// The demuxer is a slimmed-down inline copy of lib/fmp4-demux.js (the
	// Node-tested pure-function version) — the browser bundle can't
	// require() local files, so the box walker lives here. Keep the two in
	// sync; the Node version has the test coverage.
	function createWebCodecsPlayer(canvas, generation, mime, onFailure, onMetrics) {
		if (!window.VideoDecoder || !window.EncodedVideoChunk) { onFailure('VideoDecoder 不支持'); return function () {} }
		var abort = new AbortController()
		var disposed = false, reader = null, reading = false
		var decoder = null, configured = false, keyframeSeen = false
		var latestFrame = null, drawRaf = null
		var ctx = canvas.getContext('2d')
		var demuxer = createInlineFmp4Demuxer()
		var decodeQueueDepth = 0
		var MAX_DECODE_QUEUE = 8
		...
		function drawFrame() {
			...
			try { latestFrame.close() } catch (e) {}
			latestFrame = null
		}
		function scheduleDraw() {
			if (drawRaf || disposed) return
			drawRaf = window.requestAnimationFrame(drawFrame)
		}

		function pump() {
			if (disposed || reading || !reader || decodeQueueDepth >= MAX_DECODE_QUEUE) return
			reading = true
			reader.read().then(function (part) {
				reading = false
				if (disposed) return
				if (part.done) { onFailure('视频流已断开'); return }
				var events = []
				try { events = demuxer.push(part.value) }
				catch (e) { onFailure('fMP4 解复用错误: ' + e.message); return }
						...
						try {
							decoder = new VideoDecoder({
								output: function (frame) {
									// Sync canvas drawing buffer size to the decoded frame's
									// coded size. Without this, canvas defaults to 300x150
									// and drawImage scales the frame down to a tiny rect
									// (or the canvas appears empty if CSS sizing conflicts).
									if (canvas.width !== frame.codedWidth || canvas.height !== frame.codedHeight) {
										canvas.width = frame.codedWidth
										canvas.height = frame.codedHeight
									}
									if (latestFrame) try { latestFrame.close() } catch (e) {}
									latestFrame = frame
									scheduleDraw()
									if (typeof onMetrics === 'function') {
										try { onMetrics(0) } catch (e) {} // WebCodecs has no live-offset
									}
								},
							error: function (e) {
								// VideoDecoder error callback means the decoder has entered an
								// unrecoverable state (state === 'closed'). This is NOT a per-frame
								// decode failure — it kills the codec permanently. Previously this
								// was silently swallowed, leaving the player stuck in "streaming"
								// with zero decoded frames. Now we propagate to onFailure so the
								// caller can fall back to MSE.
								disposed = true
								try { abort.abort() } catch (ex) {}
								onFailure('VideoDecoder 解码失败: ' + (e && e.message || e && e.name || 'unknown error'))
							},
						})
						decoder.configure({ codec: ev.codec, description: ev.description })
						configured = true
					} catch (e) { onFailure('VideoDecoder 配置失败: ' + e.message); return }
					} else if (ev.type === 'sample' && configured) {
						if (ev.isKey) keyframeSeen = true
						if (!keyframeSeen) continue // wait for IDR to avoid decode errors
						try {
							var chunk = new EncodedVideoChunk({ type: ev.isKey ? 'key' : 'delta', timestamp: ev.timestamp, data: ev.data })
							decoder.decode(chunk)
							decodeQueueDepth += 1
						} catch (e) {
							// If decoder is closed (e.g. after an unrecoverable error),
							// stop trying to decode and propagate the failure so the
							// caller can fall back to MSE.
							var st = null
							try { st = decoder.state } catch (ex) {}
							if (st === 'closed' || !decoder) {
								onFailure('VideoDecoder 已关闭: ' + (e.message || String(e)))
								return
							}
						}
					}
				}
				// Drain: VideoDecoder.decodeQueueSize is the authoritative
				// depth; our counter is a rough proxy for backpressure.
				if (decoder) {
					try { decodeQueueDepth = decoder.decodeQueueSize } catch (e) {}
				}
				pump()
			}).catch(function (error) {
				reading = false
		...
		return function () {
			...
			if (drawRaf) { window.cancelAnimationFrame(drawRaf); drawRaf = null }
			if (latestFrame) { try { latestFrame.close() } catch (e) {} latestFrame = null }
			if (decoder) {
				// Capture locally + null outer ref first so reentrant dispose
				// calls (React strict mode renders setLiveVideo twice) can't
				// double-flush the same codec. flush() returns a Promise that
				// rejects asynchronously if the codec is closed mid-flush — a
				// bare try/catch only catches sync throws, so we check state
				// and attach a .catch() to swallow the async rejection.
				var d = decoder; decoder = null
				var st = null
				try { st = d.state } catch (e) {}
				if (st === 'configured') {
					Promise.resolve(d.flush()).catch(function () {}).then(function () { try { d.close() } catch (e) {} })
				} else {
					try { d.close() } catch (e) {}
				}
			}
	...
	// Inline fMP4 demuxer — browser-bundle copy of lib/fmp4-demux.js. Parses
	// the FFmpeg fMP4 stream into init config + per-sample NALU chunks for
	// VideoDecoder. See the Node version for the full box-walker reference;
	// this is the minimal subset (ftyp/moov/avcC/moof/trun/mdat) needed.
	function createInlineFmp4Demuxer() {
		var buffer = new Uint8Array(0)
		var initEmitted = false
		var naluLengthSize = 4
		var pendingMoof = null
		var tsCounter = 0
		var trexDefaults = { sampleSize: 0, sampleFlags: 0 }
		var frameSizeAvg = 0
		var frameCount = 0

		function readU32BE(buf, off) { return (buf[off] << 24) | (buf[off + 1] << 16) | (buf[off + 2] << 8) | buf[off + 3] }
		function readBoxHeader(buf, off) {
			...
			if (size === 1) {
				...
				// 64-bit size; assume fits in 32 bits for our use case
				size = readU32BE(buf, off + 12)
				headerSize = 16
			}
		...
		// child boxes. When we descend INTO one of these, skip 8 bytes before
		// iterating children. Only stsd matters for our avcC extraction path,
		// but we list the others for parity with lib/fmp4-demux.js.
		var FULLBOX_CONTAINERS = { stsd: true, stsz: true, stco: true, co64: true, mvhd: true, mdhd: true, tfhd: true }
		// Video sample entry types (avc1, hev1, etc.) carry a 78-byte
		// VisualSampleEntry prefix (SampleEntry header + visual fields)
		// before their child boxes. Without skipping it, findBox parses
		// the fixed fields as box headers and never sees the real avcC.
		var VISUAL_SAMPLE_ENTRY_TYPES = { avc1: true, avc3: true, hev1: true, hevc: true, hvc1: true, hvc3: true, av01: true }
		var VISUAL_SAMPLE_ENTRY_PREFIX = 78
		function findBox(buf, path) {
			...
			for (var i = 0; i < path.length; i++) {
				var target = path[i]
				var found = null
				var searchIn = current
				if (prevType && FULLBOX_CONTAINERS[prevType] && current.length >= 8) {
					searchIn = current.subarray(8)
				} else if (prevType && VISUAL_SAMPLE_ENTRY_TYPES[prevType] && current.length >= VISUAL_SAMPLE_ENTRY_PREFIX) {
					searchIn = current.subarray(VISUAL_SAMPLE_ENTRY_PREFIX)
				}
				var off = 0
				while (off < searchIn.length) {
					var h = readBoxHeader(searchIn, off)
					if (!h) break
					if (off + h.size > searchIn.length) break
					if (h.type === target && !found) found = searchIn.subarray(off + h.headerSize, off + h.size)
					off += h.size
				}
		...
		function parseAvcC(avcC) {
			if (!avcC || avcC.length < 7) return null
			var lengthSizeMinusOne = avcC[4] & 0x03
			// Slice a COPY — VideoDecoder.configure() may not correctly handle
			// Uint8Array views with non-zero byteOffset (subarrays of subarrays
			// deep in the box tree). A standalone ArrayBuffer-backed copy is safe.
			return { lengthSizeMinusOne: lengthSizeMinusOne, raw: avcC.slice() }
		}
		function codecFromAvcC(avcC) {
			if (!avcC || avcC.length < 11) return 'avc1.42E01E'
			// avcC body: [version(1)][profile(1)][compat(1)][level(1)]...
			// The codec string uses profile_idc, constraint_set flags, level_idc
			// which are bytes 1,2,3 of the avcC body (after the box header).
			var p = avcC[1], c = avcC[2], l = avcC[3]
			return 'avc1.' + [p, c, l].map(function (v) { return v.toString(16).padStart(2, '0') }).join('').toUpperCase()
		}
		function parseTrun(trun, defaults, mdatPayloadLen, moofBodyLen) {
			if (!trun || trun.length < 8) return []
			var flags = (trun[1] << 16) | (trun[2] << 8) | trun[3]
			var sampleCount = readU32BE(trun, 4)
			var off = 8
			var hasDataOffset = (flags & 0x000001) !== 0
			var hasFirstSampleFlags = (flags & 0x000002) !== 0
			var hasSampleDuration = (flags & 0x000004) !== 0 || (flags & 0x000100) !== 0
			var hasSampleSize = (flags & 0x000008) !== 0 || (flags & 0x000200) !== 0
			var hasSampleFlags = (flags & 0x000010) !== 0 || (flags & 0x000400) !== 0
			var hasCompOffset = (flags & 0x000020) !== 0 || (flags & 0x000800) !== 0
			var dataOffset = 0
			if (hasDataOffset) { if (off + 4 > trun.length) return []; dataOffset = ((trun[off] << 24) | (trun[off + 1] << 16) | (trun[off + 2] << 8) | trun[off + 3]) - (moofBodyLen + 16); off += 4 }
			if (hasFirstSampleFlags) { if (off + 4 > trun.length) return []; off += 4 }
			var samples = []
			for (var i = 0; i < sampleCount; i++) {
				if (hasSampleDuration) { if (off + 4 > trun.length) return samples; off += 4 }
				var size = 0
				if (hasSampleSize) { if (off + 4 > trun.length) return samples; size = readU32BE(trun, off); off += 4 }
				else { size = (defaults && defaults.sampleSize) || 0 }
				var sflags = 0
				if (hasSampleFlags) { if (off + 4 > trun.length) return samples; sflags = readU32BE(trun, off); off += 4 }
				else { sflags = (defaults && defaults.sampleFlags) || 0 }
				if (hasCompOffset) { if (off + 4 > trun.length) return samples; off += 4 }
				var isKey = (sflags & 0x02000000) === 0 // sample_depends_on != 2
				samples.push({ offset: dataOffset, size: size, isKey: isKey })
				dataOffset += size
			}
			// Fallback: FFmpeg empty_moov+frag_keyframe omits per-sample size
			// from trun when each fragment has a single sample. Use the entire
			// mdat payload as the sample size in that case.
		...
		function isAvcKeyframe(data, naluLengthSize) {
			...
			while (off + naluLengthSize < len) {
				var naluLen = 0
				for (var i = 0; i < naluLengthSize; i++) naluLen = (naluLen << 8) | data[off + i]
				if (naluLen <= 0 || off + naluLengthSize + naluLen > len) break
				var naluType = data[off + naluLengthSize] & 0x1f
		...
		return {
			push: function (chunk) {
				var next = chunk instanceof Uint8Array ? chunk : new Uint8Array(chunk)
				var newBuf = new Uint8Array(buffer.length + next.length)
				newBuf.set(buffer, 0); newBuf.set(next, buffer.length)
				buffer = newBuf
				var events = []
				while (buffer.length >= 8) {
					var h = readBoxHeader(buffer, 0)
					if (!h) break
					if (buffer.length < h.size) break
					var body = buffer.subarray(h.headerSize, h.size)
					var type = h.type
					if (!initEmitted) {
						if (type !== 'ftyp' && type !== 'moov' && type !== 'free' && type !== 'sidx') {
							throw new Error('unexpected MP4 init box ' + type)
						}
					if (type === 'moov') {
						var avcC = findBox(body, ['trak', 'mdia', 'minf', 'stbl', 'stsd', 'avc1', 'avcC'])
						if (avcC) {
							var parsed = parseAvcC(avcC)
							if (parsed) {
								naluLengthSize = parsed.lengthSizeMinusOne + 1
								events.push({ type: 'init', description: parsed.raw, codec: codecFromAvcC(avcC) })
								initEmitted = true
							}
						}
						// Parse trex (moov/mvex/trex) for default sample size/flags.
						// FFmpeg empty_moov omits per-sample sizes from trun.
						var trex = findBox(body, ['mvex', 'trex'])
						if (trex && trex.length >= 24) {
							trexDefaults = { sampleSize: readU32BE(trex, 16), sampleFlags: readU32BE(trex, 20) }
						}
					}
					} else {
						if (type === 'moof') {
						...
						var traf = findBox(pendingMoof, ['traf'])
						if (traf) {
							var trun = findBox(traf, ['trun'])
							if (trun) {
								var mdatPayload = body
								// Parse tfhd for per-fragment defaults (override trex)
								var defaults = trexDefaults
								var tfhd = findBox(traf, ['tfhd'])
								if (tfhd && tfhd.length >= 8) {
									var tfhdFlags = (tfhd[1] << 16) | (tfhd[2] << 8) | tfhd[3]
									var toff = 8 // skip version(1)+flags(3)+track_id(4)
									if (tfhdFlags & 0x000001) toff += 8 // base_data_offset (64-bit)
									if (tfhdFlags & 0x000002) toff += 4 // sample_desc_index
									if (tfhdFlags & 0x000008) toff += 4 // default_sample_duration
									var dSize = null, dFlags = null
									if (tfhdFlags & 0x000010) { if (toff + 4 <= tfhd.length) dSize = readU32BE(tfhd, toff); toff += 4 }
									if (tfhdFlags & 0x000020) { if (toff + 4 <= tfhd.length) dFlags = readU32BE(tfhd, toff); toff += 4 }
									defaults = { sampleSize: dSize !== null ? dSize : trexDefaults.sampleSize, sampleFlags: dFlags !== null ? dFlags : trexDefaults.sampleFlags }
								}
								var samples = parseTrun(trun, defaults, mdatPayload.length, pendingMoof.length)
								for (var si = 0; si < samples.length; si++) {
									var s = samples[si]
									if (s.size <= 0 || s.offset < 0 || s.offset + s.size > mdatPayload.length) continue
									var sampleData = mdatPayload.subarray(s.offset, s.offset + s.size)
									events.push({
										type: 'sample',
										data: sampleData.slice(),
										isKey: isAvcKeyframe(sampleData, naluLengthSize) || (frameCount > 5 && s.size > frameSizeAvg * 3),
										timestamp: tsCounter,
									})
									tsCounter += 33333
									frameSizeAvg = frameSizeAvg === 0 ? s.size : (frameSizeAvg * 0.9 + s.size * 0.1)
									frameCount++
								}
							}
						}
						pendingMoof = null
						} else if (type !== 'free' && type !== 'sidx') {
							throw new Error('unexpected MP4 media box ' + type)
						}
					}
					buffer = buffer.subarray(h.size)
				}
				return events
			},
			end: function () { if (buffer.length !== 0) throw new Error('truncated fMP4 stream') },
			reset: function () { buffer = new Uint8Array(0); initEmitted = false; naluLengthSize = 4; pendingMoof = null; tsCounter = 0; trexDefaults = { sampleSize: 0, sampleFlags: 0 }; frameSizeAvg = 0; frameCount = 0 },
		}
	}
...
/* ---- debug overlay: Phase 0 latency baseline ---- */
#dsh-ego-debug {
  padding:6px 12px; font-family:ui-monospace,Menlo,Consolas,monospace; font-size:10.5px; line-height:1.5;
  color:#ffd8d5; background:rgba(0,0,0,.4); border-bottom:1px solid rgba(255,255,255,.08);
  white-space:pre; overflow-x:auto; flex-shrink:0; max-height:120px; overflow-y:auto;
}
#dsh-ego-debug[hidden] { display:none; }
.dsh-ego-tab {
...
`;
	...
	function mountFloatingWatch(ctx) {
		...
		if (document.getElementById('dsh-ego-fab') !== null) return () => {}
				...
				<div id="dsh-ego-debug" hidden></div>
				...
				debugEl = panel.querySelector('#dsh-ego-debug')
				const FLUSH_ROUTE = '/api/ego/flush'

				// Debug overlay (Phase 0 latency baseline): double-click the title
				// to toggle a monospace strip showing CDP per-frame lag p50/p95,
				// FPS, worker dropped/sent frame counters, and FFmpeg MSE
				// live-offset. The strip refreshes 4×/s; hidden by default and
				// never shown to end users.
				const renderDebug = () => {
					if (!debugEl || debugEl.hidden) return
					const lag = lagMeter.stats()
					const off = offsetMeter.stats()
					const m = lastMetrics || {}
					const dropRate = m.sourceFrames > 0 ? Math.round((m.droppedFrames / m.sourceFrames) * 100) : 0
					const lines = []
					lines.push('backend: ' + captureBackend + '  state: ' + streamState + (streamMessage ? ' (' + streamMessage + ')' : ''))
					if (captureBackend === 'cdp') {
						lines.push('lag p50/p95/last: ' + (lag.p50 ?? '-') + '/' + (lag.p95 ?? '-') + '/' + (lag.last ?? '-') + ' ms  fps: ' + lag.fps + '  n: ' + lag.count)
						lines.push('worker: src=' + (m.sourceFrames ?? '-') + ' sent=' + (m.sentFrames ?? '-') + ' drop=' + (m.droppedFrames ?? '-') + '(' + dropRate + '%) ack=' + (m.ackErrors ?? '-'))
					} else if (captureBackend === 'ffmpeg') {
						lines.push('mse-offset p50/p95/last: ' + (off.p50 ?? '-') + '/' + (off.p95 ?? '-') + '/' + (off.last ?? '-') + ' ms  n: ' + off.count)
					}
					debugEl.textContent = lines.join('\n')
				}
				const toggleDebug = () => {
					if (!debugEl) return
					const willShow = debugEl.hidden
					debugEl.hidden = !willShow
					if (willShow) {
						if (!debugTimer) debugTimer = window.setInterval(renderDebug, 250)
						renderDebug()
					} else {
						if (debugTimer) { window.clearInterval(debugTimer); debugTimer = null }
					}
				}
				if (titleEl) titleEl.addEventListener('dblclick', toggleDebug)
				...
				// objectURL disposables for frames that arrived over the WS
				// fast path: each entry holds the URL string to revoke when the
				// same targetId is overwritten or the panel disposes. SSE
				// dataURLs never appear here (they don't need revoke).
				const frameDisposables = new Map()
				let liveImg = null
				...
				// Phase 0 latency baseline: per-frame lag (Date.now() - frame.ts)
				// for CDP, MSE live-offset for FFmpeg, plus worker-reported
				// source/sent/dropped/ackErrors metrics. Surfaced via the debug
				// overlay (double-click the panel title to toggle).
				const lagMeter = createLagMeter(60)
				const offsetMeter = createLagMeter(60)
				let lastMetrics = null
				let debugEl = null, debugTimer = null
				// WS frame fast path: once the channel delivers its first
				// binary frame, the SSE `frame` event listener short-circuits
				// to avoid double-decoding. Falls back to SSE automatically
				// when the WS drops (frameChannel onStatus flips the flag).
				let wsFrameActive = false
				...
				const applyCaptureStatus = (status) => {
					...
					if (status.metrics) lastMetrics = status.metrics
			...
			const inputSocket = createInputSocket({
				onStaleTarget: () => { liveImgTargetId = null; refresh() },
			})
			let inputBusy = false
			/**
			...
			const sendInput = (targetId, type, params) => {
				...
				inputSocket.send(targetId, type, params)
			}
			const keyboardProxy = createKeyboardProxy((targetId, type, params) => sendInput(targetId, type, params))

			// Binary frame fast path (Phase 1): WS replaces the SSE `frame`
			// event for JPEG delivery. JPEG bytes arrive as a binary WS
			// message; we wrap them in a Blob → objectURL and feed the same
			// applyFrame pipeline so zoom/pan/coordinate-mapping logic is
			// untouched. SSE stays open for spaces/capture-status/tool-call;
			// the SSE frame listener short-circuits once the WS path is live.
			const applyBinaryFrame = (targetId, jpegBuf, vw, vh, ts) => {
				if (disposed) return
				if (!jpegBuf || jpegBuf.length === 0) return
				let url = null
				try { url = URL.createObjectURL(new Blob([jpegBuf], { type: 'image/jpeg' })) } catch (e) { return }
				applyFrame(targetId, url, vw, vh, ts, true /* isObjectURL */)
			}
			if (window.WebSocket && typeof URL !== 'undefined' && typeof URL.createObjectURL === 'function') {
				frameChannel = createFrameChannel({
					onFrame: (header, jpegBuf) => applyBinaryFrame(header.targetId, jpegBuf, header.vw, header.vh, header.ts),
					onStatus: (active) => { wsFrameActive = active },
				})
			}
				/**
				...
				const makeZoomImage = (urlEl, tagName = 'img') => {
					...
					else if (tagName === 'video') { img.muted = true; img.autoplay = true; img.playsInline = true }
					// canvas (WebCodecs path): no drag/attr setup; the
					// WebCodecs player will size it on first frame.
				...
				if (captureBackend === 'ffmpeg' || cached) {
					const useWebCodecs = captureBackend === 'ffmpeg' && streamState === 'streaming' && !!window.VideoDecoder && !!window.EncodedVideoChunk
					const img = makeZoomImage(u, useWebCodecs ? 'canvas' : (captureBackend === 'ffmpeg' ? 'video' : 'img'))
					if (captureBackend === 'ffmpeg' && streamState === 'streaming') {
						stopVideo()
						const mseFallback = (msg) => {
							stopVideo()
							// Replace canvas with a video element for MSE
							if (img.tagName === 'CANVAS' && img.parentNode) {
								const videoEl = makeZoomImage(u, 'video')
								img.parentNode.replaceChild(videoEl, img)
								attachLiveImg(s.targetId, videoEl)
								videoCleanup = createMsePlayer(videoEl, streamGeneration, streamMime, (message) => { streamMessage = message }, (offsetMs) => { offsetMeter.push(offsetMs) })
							} else {
								streamMessage = msg
							}
						}
						if (useWebCodecs) videoCleanup = createWebCodecsPlayer(img, streamGeneration, streamMime, mseFallback, (offsetMs) => { offsetMeter.push(offsetMs) })
						else videoCleanup = createMsePlayer(img, streamGeneration, streamMime, (message) => { streamMessage = message }, (offsetMs) => { offsetMeter.push(offsetMs) })
					}
					else img.src = cached
					img.alt = 'live'
						...
						if (captureBackend === 'ffmpeg' && streamState === 'streaming') {
							...
							const mseFallback = (msg) => {
								...
								if (img.tagName === 'CANVAS' && img.parentNode) {
									const videoEl = makeZoomImage(u, 'video')
									img.parentNode.replaceChild(videoEl, img)
									attachLiveImg(current.targetId, videoEl)
									videoCleanup = createMsePlayer(videoEl, streamGeneration, streamMime, (message) => { streamMessage = message }, (offsetMs) => { offsetMeter.push(offsetMs) })
								} else {
									streamMessage = msg
								}
				...
				const applyFrame = (targetId, dataUrl, vw, vh, ts, isObjectURL) => {
					if (disposed) return
					if (Number.isFinite(ts)) lagMeter.push(Date.now() - ts)
					// Overwrite path: revoke the previous objectURL for this
					// targetId before swapping (the newest frame is the only
					// one that needs to stay reachable — for the live view
					// and the tab thumbnail). dataURL entries have no
					// disposable and skip this branch.
					if (isObjectURL) {
						const prev = frameDisposables.get(targetId)
						if (prev && prev !== dataUrl) { try { URL.revokeObjectURL(prev) } catch (e) {} }
						frameDisposables.set(targetId, dataUrl)
					} else if (frameDisposables.has(targetId)) {
						// SSE dataURL replacing a WS objectURL for the same
						// target: revoke the old objectURL and clear the
						// disposable so a future WS frame can re-register.
						try { URL.revokeObjectURL(frameDisposables.get(targetId)) } catch (e) {}
						frameDisposables.delete(targetId)
					}
					...
					if (frameCache.size > MAX_CACHED_FRAMES) {
						for (const [id] of frameCache) {
							...
							// Drop the disposable too (if any) and revoke.
							const d = frameDisposables.get(id)
							if (d) { try { URL.revokeObjectURL(d) } catch (e) {} frameDisposables.delete(id) }
				...
				sse.addEventListener('frame', (ev) => {
					// WS fast path active → skip the SSE base64 decode. The
					// SSE stream stays open for spaces / capture-status /
					// tool-call; only `frame` events are deduped.
					if (wsFrameActive) return
					try {
						...
						applyFrame(m.targetId, `data:image/jpeg;base64,${m.data}`, m.vw, m.vh, m.ts)
					} catch {}
					})
			...
			return () => {
				...
				inputSocket.dispose()
				if (frameChannel) { try { frameChannel.dispose() } catch (e) {} frameChannel = null }
				// Revoke any remaining objectURLs in the frame cache so a
				// long session doesn't leak Blob store entries across panel
				// re-mounts.
				for (const [, url] of frameDisposables) { try { URL.revokeObjectURL(url) } catch (e) {} }
...
`
		...
		function LivePreviewController(ctx) {
			...
			var self = this
			// Long-lived WS input channel; same lifecycle as the controller.
			// 409 / capture-target-stale from the host clears the live target
			// and re-syncs, matching the previous fetch().then(409) arm.
			this.inputSocket = createInputSocket({
				onStaleTarget: function () { self.liveImgTargetId = null; self._pointerState = null; self.refresh() },
			})
			...
			// Phase 0 latency baseline meters (same shape as the floating panel):
			// per-frame lag for CDP, MSE live-offset for FFmpeg, worker metrics
			// from capture-status. Not yet surfaced in the React UI; exposed on
			// the instance for console inspection and Phase 1/2 reuse.
			this.lagMeter = createLagMeter(60)
			this.offsetMeter = createLagMeter(60)
			this.lastMetrics = null
			// WS frame fast path (Phase 1): same Blob→objectURL→<img> pipeline
			// as the floating panel. frameDisposables tracks objectURLs per
			// targetId so overwrites and dispose revoke the previous URL.
			this.frameDisposables = new Map()
			this.wsFrameActive = false
		...
		LivePreviewController.prototype.start = function () {
			...
			this._recompute()
			// Spin up the WS frame fast path alongside the SSE stream. The SSE
			// listener short-circuits once the WS delivers its first frame;
			// if WS is unavailable we fall through to SSE-only. Same
			// Blob→objectURL→<img> pipeline as the floating panel.
			if (!this.frameChannel && window.WebSocket && typeof URL !== 'undefined' && typeof URL.createObjectURL === 'function') {
				var self = this
				this.frameChannel = createFrameChannel({
					onFrame: function (header, jpegBuf) { self.applyBinaryFrame(header.targetId, jpegBuf, header.vw, header.vh, header.ts) },
					onStatus: function (active) { self.wsFrameActive = active },
				})
			}
			if (this.visible) { this.refresh(); this.openStream() }
		}
		LivePreviewController.prototype.applyBinaryFrame = function (targetId, jpegBuf, vw, vh, ts) {
			if (this.disposed || !jpegBuf || jpegBuf.length === 0) return
			var url = null
			try { url = URL.createObjectURL(new Blob([jpegBuf], { type: 'image/jpeg' })) } catch (e) { return }
			this.applyFrame(targetId, url, vw, vh, ts, true)
		}
		LivePreviewController.prototype.dispose = function () {
			this.keyboardProxy.dispose()
			this.inputSocket.dispose()
			if (this.frameChannel) { try { this.frameChannel.dispose() } catch (e) {} this.frameChannel = null }
			if (this.frameDisposables) {
				this.frameDisposables.forEach(function (url) { try { URL.revokeObjectURL(url) } catch (e) {} })
		...
		LivePreviewController.prototype._applyCaptureStatus = function (status) {
			...
			if (status.metrics) this.lastMetrics = status.metrics
		...
		LivePreviewController.prototype._destroyVideo = function () {
			...
			// Remove the WebCodecs canvas overlay if present.
			if (this.liveCanvas) { try { this.liveCanvas.remove() } catch (e) {} this.liveCanvas = null }
		}
		LivePreviewController.prototype.setLiveVideo = function (video, targetId) {
			...
			var self = this
			// WebCodecs direct-decode path (Phase 2): if VideoDecoder is
			// available, create a canvas overlaying the <video> element and
			// decode fMP4 samples straight to VideoFrame → canvas, bypassing
			// MSE's SourceBuffer + live-seek追尾 (+100-300ms). The <video>
			// stays in the DOM for layout (zoom/pan transforms apply to it)
			// but stays empty; the canvas paints on top. Falls back to MSE
			// when VideoDecoder is unavailable or fails at runtime.
			var useWebCodecs = !!window.VideoDecoder && !!window.EncodedVideoChunk
			var mseFallback = function (message) {
				// Destroy any existing WebCodecs player + canvas overlay
				if (self.videoCleanup) try { self.videoCleanup() } catch (e) {}
				self.videoCleanup = null
				if (self.liveCanvas) { try { self.liveCanvas.remove() } catch (e) {} self.liveCanvas = null }
				// Try MSE with the <video> element
				if (window.MediaSource && window.MediaSource.isTypeSupported(self.streamMime)) {
					self.videoCleanup = createMsePlayer(video, self.streamGeneration, self.streamMime, function (msg) {
						self.streamState = 'failed'; self.streamMessage = msg; self._recompute()
					}, function (offsetMs) { self.offsetMeter.push(offsetMs) })
				} else {
			...
			if (useWebCodecs) {
				...
				canvas.className = video.className || 'dsh-ego-liveimg'
				canvas.style.position = 'absolute'
				canvas.style.left = video.offsetLeft + 'px'
				canvas.style.top = video.offsetTop + 'px'
				canvas.style.width = video.offsetWidth + 'px'
				canvas.style.height = video.offsetHeight + 'px'
				if (video.parentNode) video.parentNode.appendChild(canvas)
				this.liveCanvas = canvas
				this.videoCleanup = createWebCodecsPlayer(canvas, this.streamGeneration, this.streamMime, function (message) {
					// WebCodecs failed (decoder error or configure error) —
					// fall back to MSE instead of just marking failed.
					mseFallback(message)
				}, function (offsetMs) { self.offsetMeter.push(offsetMs) })
			} else {
				mseFallback('VideoDecoder 不支持')
			}
		}
		...
		LivePreviewController.prototype.openStream = function () {
			...
			this.sse.addEventListener('frame', function (ev) {
				// WS fast path active → skip the SSE base64 decode (same
				// dedup as the floating panel).
				if (self.wsFrameActive) return
				try {
					...
					self.applyFrame(m.targetId, 'data:image/jpeg;base64,' + m.data, m.vw, m.vh, m.ts)
				} catch (e) {}
			})
		...
		LivePreviewController.prototype.applyFrame = function (targetId, dataUrl, vw, vh, ts, isObjectURL) {
			if (this.disposed) return
			if (Number.isFinite(ts)) this.lagMeter.push(Date.now() - ts)
			// Revoke the previous objectURL for this targetId (if any) before
			// swapping — mirrors the floating panel's dispose discipline so
			// a long session doesn't leak Blob store entries.
			if (isObjectURL) {
				var prev = this.frameDisposables.get(targetId)
				if (prev && prev !== dataUrl) { try { URL.revokeObjectURL(prev) } catch (e) {} }
				this.frameDisposables.set(targetId, dataUrl)
			} else if (this.frameDisposables.has(targetId)) {
				try { URL.revokeObjectURL(this.frameDisposables.get(targetId)) } catch (e) {}
				this.frameDisposables.delete(targetId)
			}
			...
			if (this.frameCache.size > MAX_CACHED_FRAMES) {
				...
				this.frameCache.forEach(function (_, id) {
					...
					var d = self.frameDisposables.get(id)
					if (d) { try { URL.revokeObjectURL(d) } catch (e) {} self.frameDisposables.delete(id) }
				})
			}
			if (Number.isFinite(vw) && Number.isFinite(vh)) {
		...
		LivePreviewController.prototype.sendInput = function (targetId, type, params) {
			...
			this.inputSocket.send(targetId, type, params)
		}
